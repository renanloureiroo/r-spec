...
npx skills add <seu-usuario>/sdd

# Tudo, sem prompts (CI)
npx skills add <seu-usuario>/sdd --all -y

# Skills específicas para harnesses específicos
npx skills add <seu-usuario>/sdd --skill execute-qa -a claude-code -a cursor
```
...
- **Context7 MCP** — usado por `create-techspec`, `execute-task` e `execute-bugfix` para consultar documentação atualizada de linguagens/frameworks/libs.
- **Playwright MCP** (`browser_*`) — usado por `execute-qa` e `execute-bugfix` para E2E, acessibilidade e validação visual no navegador.
